...
static struct fmt_tests tests[] = {
	...
	// since this format hash has ':' chars built in, our prepare wants multiple fields.  We handle either plain
	// 'raw' fields representation, OR a hash that has a user name appeneded. i.e. SCRYPT can be in field[0]
	// or in field[1]
	{"SCRYPT1","password", {"SCRYPT","16384","8","1","VHRuaXZOZ05INWJs","JjrOzA8pdPhLvLh8sY64fLLaAjFUwYCXMmS16NXcn0A="}},
	{"SCRYPT2","test123",  {"SCRYPT","16384","8","1","bjZkemVmZ3lWVi42","cmBflTPsqGIbg9ZIJRTQdbic8OCUH+904TFmNPBkuEA="}},
	// these have a user name, to cause the SCRYPT to be in field[1] and not field[0].
	{"SCRYPT3","hello",    {"SomeUser1", "SCRYPT","16384","8","1","VlVYUzBhQmlNbk5J","bJhm6VUS2UQRwMRqLTvSsljDeq193Ge4aqQDtb94bKg="}},
	{"SCRYPT4","password", {"Some user2", "SCRYPT","16384","8","1","VHRuaXZOZ05INWJs","JjrOzA8pdPhLvLh8sY64fLLaAjFUwYCXMmS16NXcn0BhlHpZJ3J2jcozCDM7t+sfjkgQ894R+f+ldVWM5atlkA=="}},
	{NULL}
};
...
{
	static char Buf[256];
	char tmp[256], tmp4[256], tmp5[6], tmp6[6];
	int N, r, p;
	...
	if (!strncmp(fields[1], FMT_CISCO9, sizeof(FMT_CISCO9)-1)) {
		...
		sprintf (Buf, "$7$%c%s%s%14.14s$%s", N_to_c(N), tmp5, tmp6, &(fields[1][3]),
	        ...
	        (fields[0] && !strncmp(fields[0], FMT_SCRYPTKDF, sizeof(FMT_SCRYPTKDF)-1)) )
	{
		...
		int idx = 0;
		if (!strcmp(fields[1], FMT_SCRYPTKDF))
			++idx;
		if (!fields[idx+5])
			return fields[1];
		if (!isdec(fields[idx+1]) || !isdec(fields[idx+2]) || !isdec(fields[idx+3]))
			return fields[1];
		if (base64_valid_length(fields[idx+4], e_b64_mime, flg_Base64_MIME_TRAIL_EQ_CNT) != strlen(fields[idx+4]))
			return fields[1];
		if (base64_valid_length(fields[idx+5], e_b64_mime, flg_Base64_MIME_TRAIL_EQ_CNT) != strlen(fields[idx+5]))
			return fields[1];
		encode64_uint32((uint8_t*)tmp5, sizeof(tmp5), atoi(fields[idx+2]), 30);
		tmp5[5]=0;
		encode64_uint32((uint8_t*)tmp6, sizeof(tmp6), atoi(fields[idx+3]), 30);
		tmp6[5]=0;
		memset(tmp4, 0, sizeof(tmp4));
		sprintf (Buf, "$7$%c%s%s%s$%s", N_to_c(atoi(fields[idx+1])), tmp5, tmp6,
			base64_convert_cp(fields[idx+4], e_b64_mime, strlen(fields[idx+4]), tmp4, e_b64_raw, sizeof(tmp4), flg_Base64_NO_FLAGS),
			base64_convert_cp(fields[idx+5], e_b64_mime, strlen(fields[idx+5]), tmp, e_b64_cryptBS, sizeof(tmp),flg_Base64_NO_FLAGS));
	} else
		return fields[1];
	return Buf;
}

...
static void _pbkdf2_md5(const unsigned char *S, int SL, int R, ARCH_WORD_32 *out,
	...
	for(i = 1; i < R; i++) {
		...
		MD5_Final(tmp_hash, &ctx);
#if !defined (PBKDF1_LOGIC)
		for(j = 0; j < MD5_DIGEST_LENGTH/sizeof(ARCH_WORD_32); j++) {
...
{
	union {
		ARCH_WORD_32 x32[MD5_DIGEST_LENGTH/sizeof(ARCH_WORD_32)];

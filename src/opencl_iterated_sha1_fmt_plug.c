...
#ifdef HAVE_OPENCL
#define FMT_STRUCT fmt_opencl_sha1_iterated
...
#define FORMAT_LABEL        "Iterated-SHA1-opencl"
#define ALGORITHM_NAME      "SHA1 ($s.$p) OpenCL"
...
static void init_kernel(unsigned int num_ld_hashes, char *bitmap_para)
{
	...
	sprintf(build_opts, "-DMAX_SALT_SIZE=%u -DOFFSET_TABLE_SIZE=%u -DHASH_TABLE_SIZE=%u"
	...
	"-DLOC_3=%d"
#endif
	, MAX_SALT_SIZE, offset_table_size, hash_table_size, shift64_ot_sz, shift64_ht_sz,
	...
	opencl_build_kernel("$JOHN/opencl/iterated_sha1_kernel.cl",
	                    gpu_id, build_opts, 0);
...
{
	unsigned int *t = (unsigned int *) iterated_sha1_get_binary(source);

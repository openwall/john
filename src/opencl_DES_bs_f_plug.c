...
static cl_int err;
static cl_mem opencl_DES_bs_data_gpu, B_gpu, K_gpu, cmp_out_gpu, loaded_hash_gpu, bitmap, *loaded_hash_gpu_salt = NULL;
...
int opencl_DES_bs_crypt_25(int *pcount, struct db_salt *salt)
{
	...
	if (set_salt == 1) {
		...
		if (found == 0) {
			...
			HANDLE_CLERROR(clSetKernelArg(krnl[gpu_id][pos], 0, sizeof(cl_mem), &K_gpu), "Set Kernel Arg FAILED arg2\n");
			...
			HANDLE_CLERROR(clSetKernelArg(krnl[gpu_id][pos], 5, sizeof(cl_mem), &bitmap), "Set Kernel Arg krnl FAILED arg7\n");
	...
	if (salt) {
		...
		HANDLE_CLERROR(clSetKernelArg(krnl[gpu_id][pos], 2, sizeof(cl_mem), &loaded_hash_gpu_salt[current_salt]), "Set Kernel krnl Arg 3 :FAILED");
	}

	HANDLE_CLERROR(clSetKernelArg(krnl[gpu_id][pos], 3, sizeof(int), &num_loaded_hashes), "Set Kernel krnl Arg 4 :FAILED") ;

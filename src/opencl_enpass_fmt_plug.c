...
#define OCL_ALGORITHM_NAME      "PBKDF2-SHA1/512 AES OpenCL"
...
static cl_kernel pbkdf2_init, pbkdf2_loop, enpass_final;
static cl_kernel pbkdf2_512init, pbkdf2_512loop, enpass6_final;
...
static size_t get_task_max_work_group_size()
{
	...
	s = MIN(s, autotune_get_task_max_work_group_size(FALSE, 0, pbkdf2_512init));
...
static void create_clobj(size_t gws, struct fmt_main *self)
{
	...
	HANDLE_CLERROR(clSetKernelArg(pbkdf2_512init, 0, sizeof(mem_in), &mem_in), "Error while setting mem_in kernel argument");
	HANDLE_CLERROR(clSetKernelArg(pbkdf2_512init, 1, sizeof(mem_salt), &mem_salt), "Error while setting mem_salt kernel argument");
	HANDLE_CLERROR(clSetKernelArg(pbkdf2_512init, 2, sizeof(mem_state), &mem_state), "Error while setting mem_state kernel argument");
	...
	HANDLE_CLERROR(clSetKernelArg(enpass6_final, 2, sizeof(mem_state), &mem_state), "Error while setting mem_state kernel argument");
}
...
static void done(void)
{
	if (program[gpu_id]) {
		...
		HANDLE_CLERROR(clReleaseKernel(pbkdf2_512init), "Release kernel");
...
static void reset(struct db_main *db)
{
	if (!program[gpu_id]) {
		...
		HANDLE_CLERROR(ret_code, "Error creating kernel");
		enpass_final = clCreateKernel(program[gpu_id], "enpass5_final",
		                              &ret_code);
		HANDLE_CLERROR(ret_code, "Error creating kernel");
		pbkdf2_512init = clCreateKernel(program[gpu_id], "pbkdf2_sha512_kernel",
		                                &ret_code);
		HANDLE_CLERROR(ret_code, "Error creating kernel");
...
static int crypt_all(int *pcount, struct db_salt *salt)
{
	...
	if (currentsalt.version == 5) {
		...
		for (j = 0; j < (ocl_autotune_running ? 1 : (currentsalt.salt.pbk_salt.s1.outlen + 19) / 20); j++) {
			...
			BENCH_CLERROR(clEnqueueNDRangeKernel(queue[gpu_id], enpass_final, 1, NULL, &global_work_size, lws, 0, NULL, multi_profilingEvent[3]), "Run final kernel");
		}
	} else {
		/* PBKDF2-HMAC-SHA512 code path */
		int loops = (currentsalt.salt.pbk_salt.s512.rounds + HASH_LOOPS - 1) / HASH_LOOPS;

...
{
	fctx = (iwork_common_custom_salt*)salt;
	memcpy((char*)currentsalt.salt, fctx->salt, fctx->salt_length);
	currentsalt.length = fctx->salt_length;
	currentsalt.iterations = fctx->iterations;
	currentsalt.outlen = 16; // AES 128-bit key only
...
#pragma omp parallel for
		...
		if (iwork_common_decrypt(fctx, (unsigned char*)output[index].dk, fctx->iv, fctx->blob))
		{
			cracked[index] = 1;
#ifdef _OPENMP
#pragma omp atomic

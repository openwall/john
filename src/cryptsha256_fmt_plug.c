...
typedef struct cryptloopstruct_t {
	unsigned char *buf;			// will allocate to hold 42 2 block buffers (42 * 2 * 64)
	...
	int _1blk[42];				// if 1, then this is a small, only 1 block crypt. Some rounds for shorter passwords take only 1 crypt block.
								// NOTE, _1blk could be changed to a number, and then we could do > 2 block crypts. Would take a little
...
static void LoadCryptStruct(int index, char *p_bytes, char *s_bytes) {
	...
	unsigned _1b_pc, _1b_ppsc, _1b_ppc, _1b_psc; // is this 1 or 2 block
	...
	if (len_pc  <=55) {tot_pc  =64; _1b_pc  =1;}else{tot_pc  =128; _1b_pc  =0; }
	if (len_ppsc<=55) {tot_ppsc=64; _1b_ppsc=1;}else{tot_ppsc=128; _1b_ppsc=0; }
	if (len_ppc <=55) {tot_ppc =64; _1b_ppc =1;}else{tot_ppc =128; _1b_ppc =0; }
	if (len_psc <=55) {tot_psc =64; _1b_psc =1;}else{tot_psc =128; _1b_psc =0; }
...
#pragma omp parallel for
...
#endif
	{
		...
		for (cnt = 1; ; ++cnt) {
			...
			SHA256_Update(&ctx, crypt_struct[index].bufs[idx], crypt_struct[index]._1blk[idx]?64:128);

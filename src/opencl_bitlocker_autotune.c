...
static void reset(struct db_main *db)
{
	...
	if (gpu_nvidia(device_info[gpu_id])) {
		if (nvidia_sm_5x(device_info[gpu_id]))
			snprintf(build_opts, sizeof(build_opts), "-D DEV_NVIDIA_SM50=1"); //-cl-nv-verbose
		else
			snprintf(build_opts, sizeof(build_opts), "-D DEV_NVIDIA_SM50=0");
	} else
		snprintf(build_opts, sizeof(build_opts), "-D DEV_NVIDIA_SM50=0");

...
static size_t get_task_max_work_group_size()
{
	...
	s = common_get_task_max_work_group_size(FALSE, 0, GenerateSHA1pwhash);
	s = MIN(s, common_get_task_max_work_group_size(FALSE, 0, crypt_kernel));
	s = MIN(s, common_get_task_max_work_group_size(FALSE, 0, Generate2010key));
	return s;
}
...
static void init(struct fmt_main *self)
{
	...
	common_run_auto_tune(self, ITERATIONS + 4, 0,
...
static int crypt_all_benchmark(int *pcount, struct db_salt *salt)
{
	...
	HANDLE_CLERROR(clEnqueueNDRangeKernel(queue[gpu_id], GenerateSHA1pwhash, 1, NULL, &scalar_gws, &local_work_size, 0, NULL, multi_profilingEvent[e++]), "failed in clEnqueueNDRangeKernel");
	...
	HANDLE_CLERROR(clEnqueueNDRangeKernel(queue[gpu_id], Generate2010key, 1, NULL, &gws, &local_work_size, 0, NULL, multi_profilingEvent[e++]), "failed in clEnqueueNDRangeKernel");

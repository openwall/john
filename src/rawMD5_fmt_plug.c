...
#define GETPOS(i, index)		( (index&(SIMD_COEF_32-1))*4 + ((i)&(0xffffffff-3))*SIMD_COEF_32 + ((i)&3) + (index>>(SIMD_COEF_32>>1))*MD5_BUF_SIZ*4*SIMD_COEF_32 )
#else
...
static void set_key(char *_key, int index)
{
	...
	ARCH_WORD_32 *keybuffer = &((ARCH_WORD_32*)saved_key)[(index&(SIMD_COEF_32-1)) + (index>>(SIMD_COEF_32>>1))*MD5_BUF_SIZ*SIMD_COEF_32];
...
static char *get_key(int index)
{
	...
	unsigned int i;
	ARCH_WORD_32 len = ((ARCH_WORD_32*)saved_key)[14*SIMD_COEF_32 + (index&(SIMD_COEF_32-1)) + (index>>(SIMD_COEF_32>>1))*MD5_BUF_SIZ*SIMD_COEF_32] >> 3;

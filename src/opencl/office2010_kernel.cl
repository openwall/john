...
__kernel void Generate2010key(
	__global uint *pwhash, /* [gws][20/sizeof(int)] input */
	__global uint *key,    /* [gws][32/sizeof(int)] output */
	__constant uint *spincount)
{
	uint i, j;
	uint block[16], temp[5];
	uint output[5];
	uint gid = get_global_id(0);
	uint base = pwhash[gid * 6 + 5];
	uint iterations = *spincount & 1023;
...
#ifdef DEBUG
	if (gid == 0) dump_stuff_msg((__constant char*)" in", (uchar*)output, 20);
	if (gid == 0) printf((__constant char*)"Spincount: %u, remainder: %u, base: %u\n", *spincount, iterations, base);
#endif
...
#ifdef DEBUG
	dump_stuff_msg("After iterations", (uchar*)output, 20);
#endif
...
#pragma unroll
	...
	sha1_block(block, output);
#ifdef DEBUG
	dump_stuff_msg("Final 1", (uchar*)output, 20);
#endif
...
#pragma unroll
	...
	sha1_block(block, output);
#ifdef DEBUG
	dump_stuff_msg("Final 2", (uchar*)output, 20);
#endif
...
#ifdef DEBUG
	dump_stuff_msg("Final", (uchar*)key, 32);
#endif
}

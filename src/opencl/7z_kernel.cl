...
inline void sevenzip_kdf(__global const unsigned char *password, int n_password, __global const unsigned char *salt, int n_salt, int iterations, __global unsigned char *key)
{
	int i;
	int len;
	...
	for (rnd = 0; rnd < rnds; rnd++) {
		// sha256_update(&ctx, "", cur_salt->SaltSize);
		sha256_update(&ctx, buffer, len * 2);
		sha256_update(&ctx, temp, 8);
		for (i = 0; i < 8; i++)
			if (++(temp[i]) != 0)
				break;
	}
	sha256_finish(&ctx, lkey);

	__memcpy(key, lkey, 32);
}

__kernel void sevenzip(__global const sevenzip_password * inbuffer,
                  __global sevenzip_hash * outbuffer, __global const sevenzip_salt * salt)
{
	uint32_t idx = get_global_id(0);

	sevenzip_kdf(inbuffer[idx].v, inbuffer[idx].length, salt->salt, salt->length, salt->iterations, outbuffer[idx].key);
}

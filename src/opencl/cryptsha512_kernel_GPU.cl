...
void kernel_prepare(__global   sha512_salt     * salt,
    ...
    //Save results.
    tmp_mem = &alt_result[gid * 8];
    for (int i = 0; i < 8; i++)
        tmp_mem[i].mem_64[0] = SWAP64(fast_buffers.alt_result[i].mem_64[0]);
...
inline void sha512_crypt(const uint32_t saltlen, const uint32_t passlen,
	...
	for (int j = 8; j < 16; j++)
	   w[j] = 0;
        ...
        if (i & 1) {
            w[0] = work_memory[pos + 0];
            w[1] = work_memory[pos + 1];
            w[2] = work_memory[pos + 2];
            w[3] = work_memory[pos + 3];
            w[4] = work_memory[pos + 4];
            w[5] = work_memory[pos + 5];
            w[6] = work_memory[pos + 6];
            w[7] = work_memory[pos + 7];
            total = work_memory[pos + 8];
            ...
            w[6] = H[6];
            w[7] = H[7];
	    w[8] = work_memory[pos + 0];
	    w[9] = work_memory[pos + 1];
	    w[10] = work_memory[pos + 2];
	    w[11] = work_memory[pos + 3];
	    w[12] = work_memory[pos + 4];
	    w[13] = work_memory[pos + 5];
	    w[14] = work_memory[pos + 6];
	    w[15] = work_memory[pos + 7];
            total = 64 + work_memory[pos + 8];
        }
        //Initialize CTX.
	H[0] = H0;
	H[1] = H1;
...
void kernel_crypt(__constant sha512_salt     * salt,
    ...
    __global buffer_64 * buf = &alt_result[gid * 8];

    //Do the job
    sha512_crypt(salt->length, keys_buffer[gid].length, 0, HASH_LOOPS,
		 buf, &work_memory[gid * (9 * 8)]);
}
...
void kernel_final(__constant sha512_salt     * salt,
    ...
    #pragma unroll
    for (int i = 0; i < 8; i++)
        out_buffer[gid].v[i] = SWAP64(buf[i].mem_64[0]);
}
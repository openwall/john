...
inline void hmac_sha1(__private uint * output,
    ...
    __global const uchar * salt, int saltlen)
{
	int i;
	uint temp, W[16];
	uint A, B, C, D, E;
	...
	buf[saltlen] = 0x80;
	PUT_WORD_32_BE((64 + saltlen) << 3, buf, 60);
...
inline void big_hmac_sha1(__private uint * input, uint inputlen,
    __private uint * ipad_state,
    __private uint * opad_state, __private uint *output, int iterations)
{
	int i, lo;
	uint temp, W[16];
	uint A, B, C, D, E;
...
inline void pbkdf1(__global const uchar * pass, uint passlen,
                   ...
                   __global uint * out, uint outlen)
{
	...
	hmac_sha1(tmp_out, ipad_state, opad_state, salt, saltlen);
...
__kernel void derive_key(__global const pbkdf1_password *inbuffer,
    __global pbkdf1_hash *outbuffer, __global const pbkdf1_salt *salt)
{
	uint idx = get_global_id(0);

	pbkdf1(inbuffer[idx].v, inbuffer[idx].length,

	...
	char **priv_summary )
{
    ...
    *acct_status = "";
	...
	{ *acct_status = "record length too short"; return 0;  }
    if ( rec->rtype != UAF_RECTYPE_USER )
	{ *acct_status = "invalid record type"; return 0;  }
    if ( rec->usrdatoff != 0 ) {
	if ( rec->usrdatoff < UAF_REC_MIN_USRDATOFF )
		{ *acct_status = "user data out of range (short)"; return 0;  }
	if ( rec->usrdatoff > UAF_REC_SIZE )
		{ *acct_status = "user data out of range"; return 0;  }
    }
    /*
     * Fill in acct struct.
     */
    ...
    acct->username[i] = '\0';
    if ( i > 12 )
	{ *acct_status = "username too long (>12)"; return 0;  }
    acct->uic[0] = rec->uic.mem;
    acct->uic[1] = rec->uic.grp;

    len = rec->owner.len;
    if ( len > sizeof(rec->owner.s) )
	{ *acct_status = "invalid owner length"; return 0; }
    for ( i = 0; i < len; i++ ) acct->owner[i] = rec->owner.s[i];
    acct->owner[len] = '\0';

    len = rec->defcli.len;
    if ( len > sizeof(rec->defcli.s) )
	{ *acct_status = "invalid defcli length"; return 0; }
    for ( i = 0; i < len; i++ ) acct->shell[i] = rec->defcli.s[i];
    acct->shell[len] = '\0';

    len = rec->defdev.len;
    if ( len > sizeof(rec->defdev.s) )
	{ *acct_status = "invalid defdev length"; return 0; }
    for ( i = 0; i < len; i++ ) acct->home_dir[i] = rec->defdev.s[i];
    devlen = i;
    len = rec->defdir.len;
    for ( i = 0; i < len; i++ ) acct->home_dir[i+devlen] = rec->defdir.s[i];
	...
	printf ( "Bugcheck, pwdmix bitfield definition wrong: %d\n",
		rec->flags.pwdmix );
    if ( rec->flags.pwdmix ) pwd->flags |= UAI$M_PWDMIX;
    pwd->salt = rec->salt;
    pwd->alg = rec->encrypt;
    ...
    pwd2->flags = 0;
    if ( rec->flags.pwdmix ) pwd2->flags |= UAI$M_PWDMIX;
    pwd2->salt = rec->salt;
     ...
     */
    if ( rec->flags.disacnt ) *acct_status = "Disuser";
	...
	static struct {
	    ...
	    { "All",     { 0x344040a7, 0x000b} },
	    { "Objects", { 0x02220040, 0x010 } },
	    { "System",  { 0xc0053000, 0x060 } },
	    { "Devour",  { 0x09880e18, 0x000 } },
	    { "Group",   { 0x00000100, 0x004 } },
	    { "Normal",  { 0x00108000, 0} },
	    { "None",    { 0, 0 } }
	};
	for ( i = 0; priv_category[i].mask[0]; i++ ) {
	    if ( (priv_category[i].mask[0]&rec->priv[0]) ||
		 (priv_category[i].mask[1]&rec->priv[1]) ) break;
	}
...
int main ( int argc, char **argv )
{
	...
	if ( is_raw ) {
	     ...
	     */
	    if ( 1 != fread ( &rec, sizeof(rec), 1, rawf ) ) break;
	    status = uaf_extract_from_raw ( &rec, sizeof(rec), &pwd, &pwd2,
		&acct, &prefix, &priv_summary );
	} else {
	    /*
	    ...
	    if ( lf ) *lf = '\0';
	    lnum++;
	    if ( strlen(line) < 21 ) continue;   /* line too short, ignore */
	    /*
	    ...
	    if ( strlen(line) > 69 ) {
	        directory = &line[69];
	        if ( strcmp ( directory, "Disuser" ) == 0 ) prefix = "/disuser";
	        if ( strcmp ( directory, "Expired" ) == 0 ) prefix = "/expired";
	    }
	    if ( strlen(line) > 59 ) {
	     ...
	     * with colon in VMS file specification).
             */
	    if ( strcmp(username,"Username") == 0 ) continue;   /* header line */
	...
	if ( status & 1 ) {
	     ...
	     */
	    if ( UAF_QW_EQL(pwd2.hash,null_hash) ) suffix = "";
	    else suffix = ".1";	/* flag as primary of 2 */
	    ...
	    printf ("Error fetching UAF information, %s\n", prefix );
	    exit ( status );
	}
    }

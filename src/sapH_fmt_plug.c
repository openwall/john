...
static struct fmt_tests tests[] = {
	...
	// NOTE, I am not making a hash with >1 billion iterations just to get max length hash. That would take forever to process in STests.
...
static int valid(char *ciphertext, struct fmt_main *self)
{
	...
	if ((cp = strtok(NULL, " ")) == NULL)
		goto err;
	len = strlen(cp);
	len = base64_valid_length(cp, e_b64_mime, flg_Base64_MIME_TRAIL_EQ|flg_Base64_MIME_TRAIL_EQ_CNT);
...
{
	int count = *pcount, idx, i;
	for (idx = 0; idx < count; ++idx) {
		int len = strlen(saved_plain[idx]);
		unsigned char tmp[MAX_BINARY_SIZE];
...
{
	static union {
		unsigned char cp[BINARY_SIZE]; // only part the size of the hashes.

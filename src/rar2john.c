 ...
 * Usage:
 *
 * 1. Run rar2john on rar file(s) as "rar2john [rar files] > output_file".
 *    Output is written to standard output.
 * 2. Run JtR on the output generated by rar2john as "john output_file".
 *
 * Output Line Format:
 *
 * For type = 0 for files encrypted with "rar -hp ..." option
 * archive_name:$RAR3$*0*hex(salt)*hex(partial-file-contents):type::::archive_name
 *
 * For type = 2 for files encrypted with "rar -p ..." option
 * archive_name:$RAR3$*2*hex(salt)*hex(crc)*PACK_SIZE*UNP_SIZE*method*hash(blob)*base64(full encrypted file blob):type::file_name
 *
 */
...
static void process_file(const char *archive_name)
{
	...
	if (archive_header_head_flags & 0x0080) {	/* file header block is encrypted */
		...
		type = 2;	/* New always inlined version */
...
next_file_header:
	...
	if (type == 1 && file_header_block[2] == 0x7a) {
		...
		fprintf(stderr, "! %s: Not recognizing any more headers.\n", archive_name);
		goto BailOut;
	}
	...
	if (type == 0) {
		...
		/* Always store inline */
		best_len += sprintf(&best[best_len], "%c%c*", itoa16[file_header_block[25]>>4], itoa16[file_header_block[25]&0xf]);

		/* First an MD5 of the blob */
		data = mem_alloc(file_header_pack_size);
		if (fread(data, file_header_pack_size, 1, fp) != 1)
			fprintf(stderr, "Error while reading archive: %s\n",
			        strerror(errno));
		MD5_Init(&ctx);
		MD5_Update(&ctx, data, file_header_pack_size);
		MD5_Final(blobhash, &ctx);

		p = &best[best_len];
		for (i = 0; i < 16; i++) {
			*p++ = itoa16[blobhash[i] >> 4];
			*p++ = itoa16[blobhash[i] & 0xf];
		}
...
#endif
			strncat(best, gecos, LINE_BUFFER_SIZE / 2);
			puts(best);
		} else

...
static void pbkdf2_sha512_sse(const unsigned char *K[SSE_GROUP_SZ_SHA512], int KL[SSE_GROUP_SZ_SHA512], unsigned char *S, int SL, int R, unsigned char *out[SSE_GROUP_SZ_SHA512], int outlen, int skip_bytes)
{
	...
	unsigned char _sse_hash1[SHA_BUF_SIZ*sizeof(ARCH_WORD_64)*SSE_GROUP_SZ_SHA512 + MEM_ALIGN_SIMD];
	unsigned char _sse_crypt1[SHA512_DIGEST_LENGTH*SSE_GROUP_SZ_SHA512 + MEM_ALIGN_SIMD];
	unsigned char _sse_crypt2[SHA512_DIGEST_LENGTH*SSE_GROUP_SZ_SHA512 + MEM_ALIGN_SIMD];

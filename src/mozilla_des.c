...
void DES_MakeSchedule(HALF * ks, const BYTE * key, DESDirection direction)
{
	...
	if (((ptrdiff_t) key & 0x03) == 0) {
		...
		left = ((HALF) key[0] << 24) | ((HALF) key[1] << 16) | ((HALF) key[2] << 8) | key[3];
		right = ((HALF) key[4] << 24) | ((HALF) key[5] << 16) | ((HALF) key[6] << 8) | key[7];
	}
#endif
...
void DES_Do1Block(HALF * ks, const BYTE * inbuf, BYTE * outbuf)
{
	...
	if (((ptrdiff_t) inbuf & 0x03) == 0) {
		...
		left = ((HALF) inbuf[0] << 24) | ((HALF) inbuf[1] << 16) | ((HALF) inbuf[2] << 8) | inbuf[3];
		right = ((HALF) inbuf[4] << 24) | ((HALF) inbuf[5] << 16) | ((HALF) inbuf[6] << 8) | inbuf[7];
	}
...
void DES_CBCEn(struct DESContext *cx, BYTE * out, const BYTE * in, unsigned int len)
{
	...
	while (in != bufend) {
		COPY8BTOHALF((void *) {vec}, in);
		in += 8;
		vec[0] ^= cx->iv[0];
		vec[1] ^= cx->iv[1];
		DES_Do1Block(cx->ks0, (BYTE *) vec, (BYTE *) cx->iv);
		COPY8BFROMHALF(out, (void *) {
		    cx->iv});
		out += 8;
	}
}
...
SECStatus DES_Encrypt(struct DESContext *cx, BYTE * out, unsigned int *outLen, unsigned int maxOutLen, const BYTE * in,
    unsigned int inLen)
{
...
int DES_Decrypt(struct DESContext *cx, BYTE * out, unsigned int *outLen, unsigned int maxOutLen, const BYTE * in,
    unsigned int inLen)
{

...
__constant uint64_t clear_mask_be[] = {
    0xffffffffffffffffUL, 0xff00000000000000UL,	    //0,   8bits
    0xffff000000000000UL, 0xffffff0000000000UL,	    //16, 24bits
    0xffffffff00000000UL, 0xffffffffff000000UL,	    //32, 40bits
    0xffffffffffff0000UL, 0xffffffffffffff00UL,	    //48, 56bits
    0xffffffffffffffffUL                            //64    bits
};
...
#define APPEND_BE_SPECIAL(dest, src, index, start) {			       \
	...
	pos++;								       \
	offset = OFFSET(index, pos);					       \
	dest[offset] = (tmp ? (src << (64 - tmp)) : 0UL);		       \
    }									       \
}

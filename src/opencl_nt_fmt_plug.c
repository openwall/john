...
static int crypt_all(int *pcount, struct db_salt *salt)
{
	...
	if (salt != NULL && salt->count > 4500 &&
		...
		BENCH_CLERROR(clEnqueueWriteBuffer(queue[gpu_id], buffer_hash_table, CL_FALSE, 0, sizeof(cl_uint) * hash_table_size_128 * 2, hash_table_128, 0, NULL, NULL), "failed in clEnqueueWriteBuffer buffer_hash_table.");
		set_kernel_args();
		set_kernel_args_kpc();
	}

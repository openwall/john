...
#define ALGORITHM_NAME          "PKCS#12 PBE (SHA1/SHA2) OpenCL"
...
static struct fmt_tests pfx_tests[] = {
	...
	/*
	 * UTF-8 encoded password (index 4). This must be last - it will
...
static void init(struct fmt_main *_self)
{
	...
	/* The fifth test vector only works for ISO-8859-1 */
	if (options.target_enc != ISO_8859_1)
		pfx_tests[4].ciphertext = pfx_tests[4].plaintext = NULL;
}

...
static struct opt_entry opt_list[] = {
	...
	{"single-user-seed", FLG_ZERO, 0, FLG_SINGLE_CHK, OPT_REQ_PARAM,
		OPT_FMT_STR_ALLOC, &options.seed_per_user},
	...
	{"rain-strength", FLG_ZERO, 0, FLG_RAIN_CHK, OPT_REQ_PARAM,
		"%u", &options.rain_strength},
...
void opt_print_hidden_usage(void)
{
	...
	puts("--single-user-seed=FILE   wordlist with seeds per username (user:password[s] format)");

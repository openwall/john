...
#if FMT_EXTERNS_H
extern struct fmt_main fmt_opencl_rawsha512_rev;
extern struct fmt_main fmt_opencl_xsha512_rev;
#elif FMT_REGISTERS_H
john_register_one(&fmt_opencl_rawsha512_rev);
john_register_one(&fmt_opencl_xsha512_rev);
#else
...
#define FORMAT_LABEL            "raw-SHA512-rev-opencl"
#define FORMAT_NAME             ""

#define X_FORMAT_LABEL          "XSHA512-rev-opencl"
...
static void build_kernel()
{
	...
	char *task = "$JOHN/kernels/sha512_rev_kernel.cl";
...
static void prepare_bit_array()
{
	...
	do {
		...
		do {
			...
			// Skip cracked.
			if (binary) {
				SPREAD_64(binary[0], binary[0], (bitmap_size - 1U),
...
/* ------- Format structure ------- */
struct fmt_main fmt_opencl_rawsha512_rev = {
	{
		FORMAT_LABEL,
		FORMAT_NAME,
		ALGORITHM_NAME,
...
struct fmt_main fmt_opencl_xsha512_rev = {
	{
		X_FORMAT_LABEL,
		X_FORMAT_NAME,
		ALGORITHM_NAME,
		...
		sha512_common_binary_xsha512_reverse,
		get_salt,
		{NULL},
		fmt_default_source,
		{

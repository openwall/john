...
extern int mbedtls_pkcs12_derivation_simd( unsigned char *data[SSE_GROUP_SZ_SHA1],
	        ...
	        int md_type, int id, int iterations );
...
int pkcs12_pbe_derive_key_simd( int md_type, int iterations, int id, const unsigned
    ...
    int ret;
    size_t i, j;
    const unsigned char unipwd_[SSE_GROUP_SZ_SHA1][PKCS12_MAX_PWDLEN * 2 + 2], *unipwd[SSE_GROUP_SZ_SHA1];
	...
	*cpo = 0;
    ...
    if((ret = mbedtls_pkcs12_derivation_simd( key, keylen, unipwd, pwdlen,
...
int mbedtls_pkcs12_derivation_simd( unsigned char *data[SSE_GROUP_SZ_SHA1], size_t datalen,
	        ...
	        const unsigned char *salt, size_t saltlen, int md_type, int id, int iterations )
{
	...
	while( datalen > 0 )
	{
		for (k = 0; k< SSE_GROUP_SZ_SHA1; ++k) {
			...
			for (i = 0; i < SHA_DIGEST_LENGTH; ++i) {
				sse_buf[GETPOS(i, k)] = hash[i];
			}
			sse_buf[GETPOS(20,k)] = 0x80;
			sse_buf[GETPOS(63,k)] = (SHA_DIGEST_LENGTH<<3);
		}

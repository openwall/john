...
static void reset(struct db_main *db)
{
...
#if 0
	fprintf(stderr, "%s(%p) %s\n", __FUNCTION__, db, (db && db != db->real) ? "test db" : "real db");
#endif
	release_buffer();
	release_buffer_gws();
	release_kernels();
	...
	bitmap_params = prepare_table(db->salts, &offset_table,
	                              &bitmap_size_bits, &bitmaps);
	create_buffer(num_loaded_hashes, offset_table, offset_table_size,
	              hash_table_size, bitmaps, bitmap_size_bits);

	if (options.flags & FLG_MASK_CHK && (db && db->real == db)) {
		mask_mode = 1;
		fmt_opencl_lm.methods.set_key = opencl_lm_set_key_mm;
		fmt_opencl_lm.methods.get_key = get_key_mm;
	} else {
		mask_mode = 0;
		fmt_opencl_lm.methods.set_key = opencl_lm_set_key;
		fmt_opencl_lm.methods.get_key = get_key;
	}

	auto_tune_all(bitmap_params, num_loaded_hashes, 100,
	              &fmt_opencl_lm, mask_mode);
...
static int lm_crypt(int *pcount, struct db_salt *salt)
{
	...
	BENCH_CLERROR(clEnqueueWriteBuffer(queue[gpu_id], buffer_raw_keys, CL_TRUE, 0, current_gws * sizeof(opencl_lm_transfer), opencl_lm_keys, 0, NULL, NULL ), "Failed Copy data to gpu");

	if (!mask_gpu_is_static)
		BENCH_CLERROR(clEnqueueWriteBuffer(queue[gpu_id], buffer_int_key_loc, CL_TRUE, 0, current_gws * sizeof(unsigned int), opencl_lm_int_key_loc, 0, NULL, NULL ), "Failed Copy data to gpu");

	BENCH_CLERROR(clEnqueueNDRangeKernel(queue[gpu_id], crypt_kernel, 1, NULL, &current_gws, lws, 0, NULL, &evnt), "Failed enqueue kernel lm_bs_*.");

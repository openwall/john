	...
	while (bMore) {
		...
		for (i = 0; i < MD5_LOOPS; ++i) {
			if (cnt == loops[i]) {
				...
				for (j = 0; j < 4; ++j) {
					((ARCH_WORD_32*)out)[(i<<(SIMD_COEF_32>>1))+j] = a[(j<<(SIMD_COEF_32>>1))+offx];
				}
			} else if (cnt < loops[i])
				bMore = 1;
		}
	...
	while (bMore) {
		...
		for (i = 0; i < SHA1_LOOPS; ++i) {
			if (cnt == loops[i]) {
				...
				for (j = 0; j < 4; ++j) {
					((ARCH_WORD_32*)out)[(i<<(SIMD_COEF_32>>1))+j] = JOHNSWAP(a[(j<<(SIMD_COEF_32>>1))+offx]);
				}
			} else if (cnt < loops[i])
				bMore = 1;
		}
	...
	while (bMore) {
		...
		for (i = 0; i < SHA256_LOOPS; ++i) {
			if (cnt == loops[i]) {
				// only 16 bytes.
				for (j = 0; j < 4; ++j) {
					((ARCH_WORD_32*)out)[(i<<(SIMD_COEF_32>>1))+j] = JOHNSWAP(a[(j<<(SIMD_COEF_32>>1))+i]);
				}
			} else if (cnt < loops[i])
				bMore = 1;
		}
...
static void DoSHA256_crypt_sse(void *in, int ilen[SIMD_COEF_32], void *out[SIMD_COEF_32], unsigned int *tot_len, int isSHA256, int tid) {
	...
	while (bMore) {
		...
		for (i = 0; i < SHA256_LOOPS; ++i) {
			if (cnt == loops[i]) {
				for (j = 0; j < 8; ++j) {
					y.a[j] =JOHNSWAP(a[(j<<(SIMD_COEF_32>>1))+i]);
				}

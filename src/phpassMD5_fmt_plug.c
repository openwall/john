...
static struct fmt_main *self_Dynamic_17;
...
{
	if (text_in_dynamic_format_already(self_Dynamic_17, ciphertext))
		return ciphertext;
...
static int phpassmd5_valid(char *ciphertext, struct fmt_main *self)
{
		...
		if (strlen(ciphertext) != CIPHERTEXT_LENGTH) {
			return self_Dynamic_17->methods.valid(ciphertext, self_Dynamic_17);
		}
		...
		return self_Dynamic_17->methods.valid(Convert(Conv_Buf, ciphertext), self_Dynamic_17);
}

static void * our_salt(char *ciphertext)
{
	get_ptr();
	return self_Dynamic_17->methods.salt(Convert(Conv_Buf, ciphertext));
}
static void * our_binary(char *ciphertext)
{
	return self_Dynamic_17->methods.binary(Convert(Conv_Buf, ciphertext));
}
...
static void phpassmd5_init(struct fmt_main *self)
{
	if (self->private.initialized == 0) {
		self_Dynamic_17 = dynamic_THIN_FORMAT_LINK(&fmt_phpassmd5, Convert(Conv_Buf, phpassmd5_tests[0].ciphertext), "phpass", 1);
		fmt_phpassmd5.methods.salt   = our_salt;
		fmt_phpassmd5.methods.binary = our_binary;
		fmt_phpassmd5.methods.split = our_split;
		fmt_phpassmd5.params.algorithm_name = self_Dynamic_17->params.algorithm_name;
...
static void get_ptr() {
	if (!self_Dynamic_17) {
		self_Dynamic_17 = dynamic_THIN_FORMAT_LINK(&fmt_phpassmd5, Convert(Conv_Buf, phpassmd5_tests[0].ciphertext), "phpass", 0);
		fmt_phpassmd5.methods.salt   = our_salt;
		fmt_phpassmd5.methods.binary = our_binary;
		fmt_phpassmd5.methods.split = our_split;
	}

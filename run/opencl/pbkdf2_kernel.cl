...
static inline void SHA1(__private uint *A, __private uint *W) {
#if HAVE_LUT3
...
static inline void SHA1_digest(__private uint *A, __private uint *W) {
#if HAVE_LUT3
#define F(x, y, z) lut3(x, y, z, 0xca)
#elif USE_BITSELECT
#define F(x, y, z) bitselect(z, y, x)
...
static inline void sha1_pad(__private uint *pad, __private uint *state) {
	uint 		A[5], W[16] ;
...
static inline void hmac_sha1(__private uint *istate, __private uint *ostate, __private uint *buf)
{
	uint	 A[5], W[16] ;

...
/*
 * SHA384 differs from SHA512 only in the initial hash values and the
 * truncated output (6 x uint64_t = 48 bytes instead of 8 x 64 = 64).
 * The round constants, Sigma/sigma functions, and block logic are identical.
 * We override H0-H7 (defined by opencl_sha512.h via opencl_rawsha512.h)
 * with the SHA384 initial values here.
 */
 ...
 */
INLINE void sha384_block(	  const uint64_t * const buffer,
...
void kernel_crypt(
    ...
    /* Run the collected hash value through sha384. */
    sha384_block(w, total, H);

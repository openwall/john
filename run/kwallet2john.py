...
def process_file(filename):
    ...
    if encrypted_size % 8 != 0:
        sys.stderr.write(stderr, "%s : invalid file structure!\n", filename)
        sys.exit(7)

    sys.stdout.write("%s:$kwallet$%ld$%s" % (os.path.basename(filename),
        encrypted_size, hexlify(encrypted)))

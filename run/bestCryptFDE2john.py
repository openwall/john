...
def main(args):
    offset = args.offset
    bc_file = open(args.disk_image, 'rb')
    bc_data = bc_file.read(0x512 + offset)
    bc_file.close()
    if bc_data[0x1fe:0x200] == b'\x55\xaa':
        for record in range(0, 48, 16):
            offset += (0x200 * struct.unpack('<i', bc_data[0x1c6 + record:0x1ca + record])[0])
            get_metadata(args.disk_image, record + offset)

    else:
        get_metadata(args.disk_image, 0)

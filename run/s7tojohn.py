...
Extended by Narendra Kangralkar <narendrakangralkar at gmail.com> and Dhiru Kholia <dhiru at openwall.com>

S7 protocol, is used for communication between Engineering Stations, SCADA, HMI and PLC can be
...
def get_challenge_response(cfg_pcap_file):
    ...
    if raw_challenge[46:52] == '100214' and raw_challenge[92:94] == '00':
        ...
        print >> sys.stderr, "cannot find challenge for %s. exiting..." % cfg_pcap_file
        return
    ...
    if raw_response[64:70] == '100214' and raw_response[110:112] == '00':
        ...
        print >> sys.stderr, "cannot find response for %s. exiting..." % cfg_pcap_file
        return
    ...
    if pckt_84:
        ...
        outcome = 0
    print "%s:$s7$%s$%s$%s" % (cfg_pcap_file, outcome, challenge, response)


def process_file(cfg_pcap_file):
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print >> sys.stderr, "Usage: %s <.pcap files>" % sys.argv[0]
        sys.exit(-1)

    for i in range(1, len(sys.argv)):
        data = get_challenge_response(sys.argv[i])

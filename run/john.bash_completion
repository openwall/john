...
_john()
{
	...
	local subformats list dir cmd cmd2 i ver ver1 ver2 ver3 prev words prefix option
...
#	The same applies for the --show and --single options)
	...
	case "${option}" in
		...
		--wordlist=*|--mkv-stats=*)
			...
			# I can just reuse the --wordlist= logig for --mkv-stats=*
			cur=${cur#*[=:]}
			 __expand_tilde_by_ref cur 2>/dev/null
			_filedir
			return 0
		...
		--markov=*)
			...
			if [[ "${valopts}" == *--list=* ]] ; then
				...
				cur=`echo ${cur#*[=:]} | tr A-Z a-z`
				COMPREPLY=( $(compgen -W "${list} LEVEL[:START[:END[:LENGTH]]] MODE MODE:LEVEL[:START[:END[:LENGTH]]]" -- ${cur}) )
		...
		--salts=*)
			...
			return 0
			;;
		--encoding=*|--intermediate-enc=*|--internal-encoding=*|--target-encoding=*|--input-encoding=*)
			cur=${cur#*[=:]}
			...
			encodings=`${cmd} 2>&1|grep -v 'Supported encodings'|sed 's#[,)]##g'|sed 's#(or ##g'|tr A-Z a-z`

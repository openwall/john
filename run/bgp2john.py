...
def pcap_parser(fname):
    ...
    for _, buf in pcap:
        ...
        if eth.type == dpkt.ethernet.ETH_TYPE_IP:
            ...
            found = False
            for i in range(0, len(tcp.opts) - 18 + 1):
                # TCP_OPT_MD5 = 19 implies TCP MD5 signature, RFC 2385
                if ord(tcp.opts[i]) == 19:
                    found = True
                    break
            ...
            # MD5 signature "option" is 18 bytes long
            if ord(tcp.opts[i+1]) != 18:
                continue

            opt_type, opt_data = dpkt.tcp.parse_opts(tcp.opts[i:])[0]
